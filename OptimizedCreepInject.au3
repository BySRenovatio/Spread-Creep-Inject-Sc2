...
While 1
	If $timer = 110 AND $startToCreep = 1 Then
		$objsp.Speak("C")
	EndIf
	
	If $timer >= 220 AND Mod($timer, 80) = 0 AND $startToCreep = 1 Then
		$objsp.Speak("Creep")
	EndIf
...
Func remapOn9()
	...
	Do
		...
		Send("88")
		Sleep(50)
		MouseClick($MOUSE_CLICK_LEFT, $xCenter, $yCenter, 2, 0)
	...
	Do
		...
		If $pCreepSpreadColor = 15659684 Then
			...
			For $k=8 to 2 step -1
				Send("88")
				Send("C")
				MouseClick($MOUSE_CLICK_LEFT, $xCenter + $k * $xSpreadOffset, $yCenter + $k * $ySpreadOffset, 1, 0)
				If $xSpreadOffset = 0 Or $ySpreadOffset = 0 Then
					If $xSpreadOffset = 0 Then
						MouseClick($MOUSE_CLICK_LEFT, $xCenter + 180, $yCenter + $k * $ySpreadOffset, 1, 0)
						MouseClick($MOUSE_CLICK_LEFT, $xCenter - 180, $yCenter + $k * $ySpreadOffset, 1, 0)
					EndIf
					If $ySpreadOffset = 0 Then
						MouseClick($MOUSE_CLICK_LEFT, $xCenter + $k * $xSpreadOffset, $yCenter + 180, 1, 0)
						MouseClick($MOUSE_CLICK_LEFT, $xCenter + $k * $xSpreadOffset, $yCenter - 180, 1, 0)
					EndIf
				Else
					MouseClick($MOUSE_CLICK_LEFT, $xCenter + $k * $xSpreadOffset / 2, $yCenter + $k * $ySpreadOffset, 1, 0)
					MouseClick($MOUSE_CLICK_LEFT, $xCenter + $k * $xSpreadOffset, $yCenter + $k * $ySpreadOffset / 2, 1, 0)
				EndIf
			Next
		EndIf		
...
Func spread()
	...
	If $SpreadOrRemap = 1 Then
		...
		remapOn9()
		restoreScreen()
	ElseIf $spreadOrRemap = 2 Then
		$spreadOrRemap = 3
		saveScreen()
		spreadCreep()
		restoreScreen()
		$timer = 0
	ElseIf $spreadOrRemap = 3 Then
		$spreadOrRemap = 1
		saveScreen()
		remapOn8()
		restoreScreen()
